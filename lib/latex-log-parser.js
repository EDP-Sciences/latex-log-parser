define(function() {
    ...
    // Change filename
    var OPEN_FILE_REGEX = /^\(\s*([\.\/].*?)(\)?)$/; //not sure it's OK on MS Windows...
    var CLOSE_FILE_REGEX = /^\)((?:\)|\s+\().*)?$/;
    ...
    (function() {
        this.parse = function() {
            while ((this.currentLine = this.log.nextLine()) !== false) {
                //console.warn("[" + this.log.row + "][" + this.currentFilePath + "]" + this.currentLine);
                this.parseLaTeXMsg()
                    || this.parseTeXError()
                    || this.parseVHbox()
                    || this.parseOpenFile()
        ...
        this.parseOpenFile = function() {
            var matches = null;
            if (matches = this.currentLine.match(OPEN_FILE_REGEX)) {
                filePath = matches[1];
                //console.warn("OPEN " + filePath);
        ...
        this.parseCloseFile = function() {
           var matches = null;
            if (matches = this.currentLine.match(CLOSE_FILE_REGEX)) {
                this._closeFile();
                if (matches[1]) { // continue the process
                    this.currentLine = this.currentLine.replace(/^\)\s*/,'');
                    this.parseOpenFile() || this.parseCloseFile();
                }
                return true;
            }
            return false;
        ...
        this._closeFile = function() {
            if (this.fileStack.length > 1) {
                //console.warn(this.currentFilePath + " CLOSED");
                this.fileStack.pop();
                ...
                //console.warn("WARN: close paren while no file open");
            }
        }        
        ...
        this._getErrMsg = function(type) {
            ...
            if (type === 'latex'){
                msg +=  "\n"
                    + this.log.linesUntilLineMatch(/^(\s*|\[\d)$/).join("\n");
            }
            return  [l,msg];
        }
